...
const ENDPOINTS = [
  ...
  { method: "GET", path: "/records?track=500&mode=adjusted", title: "Racing records", desc: "Fastest finishes per distance from resolved races, on-chain times. mode=raw or adjusted (adjusted normalizes finish time to average track conditions, validated out of sample; the condition each record was set in is always shown). window=all, weekly, or daily. Params track, mode, window, limit (default 25, max 100), offset. Each row carries petId, name, resolved ownerName, rawTimeMs, adjustedTimeMs (null if the adjustment did not validate), raceTemp, raceId, resolvedAt." },
