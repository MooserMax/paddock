...
const ENDPOINTS = [
  ...
  { method: "GET", path: "/stables?limit=10", title: "Stable leaderboard", desc: "Wallets ranked by proven roster quality, the shrunk average confirmed quality of each stable's proven horses. Proven roster quality, not racing skill, not value. Each row carries ownerAddress, resolved ownerName, score, percentile, provenCount, totalHorses, and avgProvenCq. Paginated with limit (default 25, max 100) and offset." },
  { method: "GET", path: "/records?track=500&mode=adjusted", title: "Racing records", desc: "Fastest finishes per distance from resolved races, on-chain times, and the programmatic export of the records board. mode=raw or adjusted (adjusted corrects for track temperature on tracks where it is board-fair out of sample; this reduces, but does not fully remove, condition effects, so the condition is always shown). window=all, weekly, or daily. Params track, mode, window, limit (default 25, max 100), offset. Response carries adjustmentApplied per the selected track, adjustedTracks, and fastest; each row carries petId, name, rarity, resolved ownerName, rawTimeMs, adjustedTimeMs (null where that track is not adjusted), raceTemp, raceId, resolvedAt." },
  ...
  { method: "GET", path: "/lobbies", title: "Race Finder, live lobbies", desc: "Open forming races right now, read-only and live (polled politely, cached a few seconds, so it may lag). Each lobby carries trackLength, raceTemp (null while forming), fieldSize, petCount, openSlots, entryFeeWei, payoutBps, and the current entrants with strength. Add ?wallet={address} or ?pet={id} and each lobby also carries your win edge as an honest band (the live model adds ELO and fit signals that are not validated at these odds, so it is a banded label, not a precise percent), an EV estimate, and recommended horse. Forming-field odds shift as horses enter and are uncalibrated estimates, never guarantees." },
